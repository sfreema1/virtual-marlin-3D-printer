  ...
  if (code_seen('G')){
    ...
    case 1:
      if (code_seen('S')) {
        ...
        Serial.print("Time to execute (s): "); 
        Serial.println((step_interval)*(float)code_num/500.0);
    ...
    case 2:
      ...
      return;
      break;
    case 100: // Change the step interval
      if (code_seen('S')){
        code_num = code_value();
        Serial.println("Code seen was G100 (Set step interval time (ms).)");
        step_interval = code_num;
        Serial.print("The step internal time was set to "); Serial.print(step_interval); Serial.println(" ms.");
      }
